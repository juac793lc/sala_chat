...
class AudioPlaylistService {
  ...
  void updateAudioList(List<Comentario> audioComments) {
    ...
    for (int i = 0; i < _audioComments.length; i++) {
      ...
      debugPrint('   $i: $slice${preview.length > 30 ? '...' : ''}');
    }
  }
  ...
  Future<void> playAudioAtIndex(int index) async {
    try {
      ...
      debugPrint('📊 Estado actual: _isPlaying=$_isPlaying, _currentIndex=$_currentIndex');
      debugPrint('📊 Lista tiene ${_audioComments.length} audios');
      debugPrint('📊 Índice solicitado: $index');
      ...
      debugPrint('🔊 Player state antes: ${_audioPlayer.state}');
      ...
      if (effectiveUrl.isEmpty) {
        debugPrint('⚠️ URL vacía para audio id=${audio.id}, abortando reproducción');
        return;
      }
      
      debugPrint('⏹️ Deteniendo audio actual...');
  ...
  final src = effectiveUrl;
      if (kIsWeb) {
        debugPrint('🌐 (web) Usando SIEMPRE HTMLAudioElement (forzado)');
      ...
      debugPrint('❌ Error reproduciendo audio index=$index: $e');
      _isPlaying = false;
      _notifyPlaybackStateChanged();
    }
  }
  ...
  Future<void> _playNative(String path) async {
    if (path.startsWith('http')) {
      debugPrint('🌐 (native) URL: $path');
      await _audioPlayer.play(UrlSource(path));
    } else {
      final file = File(path);
      if (await file.exists()) {
        debugPrint('� (native) Archivo local: $path');
        await _audioPlayer.play(DeviceFileSource(path));
      } else {
        debugPrint('🔄 (native) Intentando como URL fallback');
        await _audioPlayer.play(UrlSource(path));
      }
    }
  }
  ...
  Future<void> _playHtmlElement(String url, {bool force=false}) async {
    ...
    audio
      ...
      ..controls = false
      ..autoplay = false;
    debugPrint('🎧 (html5) Preparando elemento para: $url');
    ...
    Future<void> waitReady(html.AudioElement el) async {
    ...
    try {
      ...
      await playFuture.catchError((err) => debugPrint('⚠️ (html5) play() rechazado: $err'));
      if (audio.paused) {
        await Future.delayed(const Duration(milliseconds: 120));
        try { await audio.play(); } catch (e) { debugPrint('⚠️ Segundo intento play() falló: $e'); }
      }
      if (!audio.paused) {
        debugPrint('✅ (html5) Reproduciendo');
        _isPlaying = true;
        _notifyPlaybackStateChanged();
        audio.onEnded.first.then((_) => _playNext());
        audio.onError.first.then((_) => debugPrint('❌ (html5) error evento'));
        return; // éxito
      }
      throw Exception('No se pudo iniciar reproducción (autoplay / permiso / codec)');
    } catch (e) {
      debugPrint('❌ (html5) intento directo falló: $e');
      // Fallback: fetch -> blob -> object URL
      if (url.startsWith('http')) {
        try {
          debugPrint('🔄 (fallback) Descargando $url para reproducir como blob');
          final request = await html.HttpRequest.request(url,
              method: 'GET', responseType: 'blob');
          final blob = request.response as html.Blob;
          debugPrint('📦 (fallback) Blob recibido: ${blob.size} bytes, type=${blob.type}');
          ...
          debugPrint('🔗 (fallback) Object URL creado');
          ...
          a2
            ...
            ..controls = false;
          await waitReady(a2);
          try { await a2.play(); } catch (e2) { debugPrint('⚠️ (fallback) play() rechazado: $e2'); }
          if (a2.paused) {
            await Future.delayed(const Duration(milliseconds: 120));
            try { await a2.play(); } catch (_) {}
          }
          if (!a2.paused) {
            debugPrint('✅ (fallback) Reproduciendo desde blob');
            _isPlaying = true;
            _notifyPlaybackStateChanged();
            a2.onEnded.first.then((_) => _playNext());
            a2.onError.first.then((_) => debugPrint('❌ (fallback) error evento'));
            return;
          } else {
            debugPrint('❌ (fallback) tampoco se pudo reproducir blob');
            rethrow;
          }
        } catch (f) {
          debugPrint('❌ Fallback fetch->blob falló: $f');
          rethrow;
        }
      } else {
        rethrow;
  ...
  Future<void> _playNext() async {
    debugPrint('🔄 _playNext(): Índice actual $_currentIndex, Total audios: ${_audioComments.length}');
    
    if (_currentIndex < _audioComments.length - 1) {
      // Hay más audios, reproducir el siguiente
      final nextIndex = _currentIndex + 1;
      debugPrint('➡️ Reproduciendo siguiente audio en índice: $nextIndex');
      await playAudioAtIndex(nextIndex);
    } else {
      // No hay más audios, detener reproducción
      debugPrint('🏁 Lista de reproducción completada');
      _currentIndex = -1;
      _isPlaying = false;
      _currentPosition = Duration.zero;
      _totalDuration = Duration.zero;
